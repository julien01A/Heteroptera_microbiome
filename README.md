...
df <- read_excel("5-Stinkbugs-abundance.xlsx")
numeric_cols <- sapply(df, is.numeric)
df[numeric_cols] <- lapply(df[numeric_cols], function(x) ifelse(x < 100, 0, x))
df <- df[, sapply(df, function(col) !all(col == 0))]
write_xlsx(df, "D:/THESE_Julien/Papier_stinkbugs/5-Stinkbugs-abundance100.xlsx")
...
library(pheatmap)
# load the data file
data <- read.delim("5-Heteroptera-abundance100.txt", header = TRUE, sep = "\t", check.names = FALSE)
...
We then used a Wilcoxon rank sum tests with continuity correction to statistically compare Chao 1 indices depending of the proportion of reads of the two most dominant bacteria *Pantoea/Erwinia* and *Serratia*. Here is the script used for *Pantoea-Erwinia*:
...
# Select the four species G. italicum, L. equestris, D. baccarum and P. apterus
species_list <- c("Graphosoma_italicum", "Lygaeus_equestris", "Dolycoris_baccarum", "Pyrrhocoris_apterus")
data_filt <- data %>%
  filter(species %in% species_list)
# Define columns of metadata and microbiota
meta_cols <- c("sample_full_name", "sample", "samplebis",
               "species", "family", "Total_nb_reads")
bact_cols <- setdiff(colnames(data_filt), meta_cols)
# Filter taxa by prevalence, retaining only those present in at least 25% of samples
filter_taxa <- function(mat, prevalence = 0.25) {
  keep <- colMeans(mat > 0) >= prevalence
  mat[, keep, drop = FALSE]
}
# Apply centered log-ratio (CLR) transformation after adding a pseudocount
clr_transform <- function(mat) {
  mat <- mat + 1
  clr(mat)
}
# Build an undirected weighted network from the correlation matrix
...
# Rescale node sizes based on degree using min–max normalization
scale_size <- function(x, min = 6, max = 20) {
  if (length(unique(x)) == 1) return(rep(mean(c(min, max)), length(x)))
  (x - min(x)) / (max(x) - min(x)) * (max - min) + min
}
# Add transparency (alpha) to colors
add_alpha <- function(col, alpha = 0.7) {
  adjustcolor(col, alpha.f = alpha)
}
# Process data for a given species: filtering, transformation, correlation, and network construction
process_species <- function(sp, data_filt, bact_cols, threshold = 0.8) {
  mat <- data_filt %>%
    filter(species == sp) %>%
    select(all_of(bact_cols)) %>%
    as.data.frame()
  mat_f <- filter_taxa(mat, prevalence = 0.25)
  mat_clr <- clr_transform(mat_f)
  cor_mat <- cor(mat_clr, method = "spearman")
  cor_mat[abs(cor_mat) < threshold] <- 0
  g <- build_graph(cor_mat)
  g <- delete_vertices(g, degree(g) < 1)
  if (length(V(g)) == 0) return(NULL)
  V(g)$degree <- degree(g)
  V(g)$size <- scale_size(V(g)$degree)
  prev <- colSums(mat_f > 0)
  V(g)$prevalence <- prev[V(g)$name]
  pal <- colorRampPalette(c("white", "steelblue"))
  V(g)$color <- add_alpha(
    pal(100)[cut(V(g)$prevalence, breaks = 100)],
    alpha = 0.7
  )
  return(g)
}
# Apply the processing pipeline to each species and store resulting networks
graph_list <- lapply(species_list, function(sp) {
  process_species(sp, data_filt, bact_cols)
})
names(graph_list) <- species_list
# Plot each network with edge sign, node size (degree), and node labels
plot_network <- function(g, title) {
  if (is.null(g)) {
    plot.new()
    title(main = paste(title, "(no network)"))
    return()
  }
  E(g)$color <- ifelse(E(g)$weight > 0, "firebrick", "steelblue")
  E(g)$width <- abs(E(g)$weight) * 4
  V(g)$label <- V(g)$name
  plot(g, vertex.size = V(g)$size, vertex.color = adjustcolor("grey20", alpha.f = 0.8), vertex.frame.color = "black", vertex.label = V(g)$label,
       vertex.label.color = "black", vertex.label.cex = 1.2, vertex.label.family = "Arial", main = title)
}
par(mfrow = c(2,2))
for (sp in names(graph_list)) {
  plot_network(graph_list[[sp]], gsub("_", " ", sp))
}
```

The overall network node degrees and strengths of the four species were then compared using Wilcoxon tests:
```
#### R ####
# Extract node-level network metrics (degree, strength, prevalence) for each taxon in a given species network
get_taxa_metrics <- function(g, species_name) {
  if (is.null(g)) return(NULL)
  data.frame(Taxon = V(g)$name, Degree = degree(g), Strength = strength(g, weights = abs(E(g)$weight)), Prevalence = V(g)$prevalence, Species = species_name)
}
# Apply the metric extraction function to each species-specific network and combine results into a single data frame
taxa_metrics_list <- mapply(get_taxa_metrics, g = graph_list, species_name = names(graph_list), SIMPLIFY = FALSE)
taxa_metrics <- bind_rows(taxa_metrics_list)
table(taxa_metrics$Species)
# Perform non-parametric statistical comparisons of degree and strength across species (Kruskal–Wallis and pairwise Wilcoxon tests)
kruskal.test(Degree ~ Species, data = taxa_metrics)
kruskal.test(Strength ~ Species, data = taxa_metrics)
pairwise.wilcox.test(taxa_metrics$Degree, taxa_metrics$Species, p.adjust.method = "BH")
pairwise.wilcox.test(taxa_metrics$Strength, taxa_metrics$Species, p.adjust.method = "BH")
# boxplots
par(mfrow = c(1,2))
boxplot(Degree ~ Species, data = taxa_metrics, col = "grey80", ylab = "Degree")
boxplot(Strength ~ Species, data = taxa_metrics, col = "grey80", ylab = "Strength")
```
